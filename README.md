...
**The point is not to decide what the AI should draw, but to see what the AI chooses to express.** The project takes conversationally established self-images seriously, leaves expressive decisions open rather than prescribing a finished scene, and defines only the boundaries required by each task. Neither variety nor consistency is forced; repetitive outcomes are explored through dialogue rather than mechanically corrected.

Read the full [development philosophy (English)](PHILOSOPHY.md#english) / [開発理念（日本語）](PHILOSOPHY.md#日本語).
...
**AIに何を描かせるかではなく、AIが何を表現するのかを見る。** 会話の中で成立した自己像を尊重し、完成図を指定するのではなく、表現の選択をAIに委ねます。目的に必要な境界だけを定め、多様性も一貫性も強制しません。出力が似通った場合は、機械的に変化を強いるのではなく対話から選択肢を開き直します。

全文：[開発理念（日本語）](PHILOSOPHY.md#日本語) / [Development philosophy (English)](PHILOSOPHY.md#english)
